...
public abstract class PoolSizeCalculator {
   ...
   *                          1) 	 * @param targetQueueSizeBytes 	 *            the desired
   *                          maximum work queue size of the thread pool (bytes)
   */
  ...
  private void calculateOptimalCapacity(BigDecimal targetQueueSizeBytes) {
    ...
    BigDecimal queueCapacity = targetQueueSizeBytes
        .divide(new BigDecimal(mem), RoundingMode.HALF_UP);
  ...
  private void calculateOptimalThreadCount(long cpu, long wait, BigDecimal targetUtilization) {
    ...
    System.out.println(
        "Formula: " + numberOfCPU + " * " + targetUtilization + " * (1 + " + waitTime + " / "
            + computeTime + ")");
    System.out.println("* Optimal thread count: " + optimalthreadcount);
  }

  /**
   * Runs the {@link Runnable} over a period defined in {@link #testtime}. 	 * Based on Heinz
   * Kabbutz' ideas 	 * (http://www.javaspecialists.eu/archive/Issue124.html). 	 *  	 * @param task
